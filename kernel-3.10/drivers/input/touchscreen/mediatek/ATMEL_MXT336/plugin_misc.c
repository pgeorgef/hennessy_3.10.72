...
/****************************************************************  
	Pitter Liao add for macro for the global platform
		...
		mobile: 13244776877
-----------------------------------------------------------------*/
#define PLUG_MISC_VERSION 0x0015
/*----------------------------------------------------------------
fixed some bugs
0.15:
add default id name support
0.14:
support fw postfix
0.13
ptc tune at config updated
0.12
...
#define MSC_STEP_PTC_SET_PARA			(1<<0)
#define MSC_STEP_PTC_IDLE				(1<<1)
...
struct ptc_config {
	...
	s16 target;
	unsigned long interval_param_set_check;
	s16 max_count;
};
...
static void plugin_misc_hook_t6(struct plugin_misc *p, u8 status)
{
	...
	dev_info2(dev, "mxt msc flag=0x%lx %x\n",
		 obs->flag, status);
}
...
static void msc_ptc_reset_param(struct plugin_misc *p, struct ptc_status *sts, unsigned long flag)
{
	...
	if (test_flag(PTC_STATUS_CONTINUE, &flag)) {
		...
		set_flag(MSC_STEP_PTC_IDLE, &sts->step);
	}
}
...
static ssize_t misc_psd_show(struct plugin_misc *p, char *buf, size_t count)
{
	...
	dev_info(dev, "[mxt]psd status: Flag=0x%08lx\n",
		psd_obs->flag);
...
static ssize_t misc_pid_show(struct plugin_misc *p, char *buf, size_t count)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct device *dev = dcfg->dev;
	int offset = 0;
	...
	if (ret == 0) {
		...
		offset += scnprintf(buf, count, "PID name: %s\n", name);
	}

	return offset;
}

static int plugin_misc_get_pid_val(struct plugin_misc *p, u8 *id, int len)
{
	struct msc_observer *obs = p->obs;
	struct psd_observer *psd_obs = &obs->psd;
	int ret = -ENODEV;

	if (!id || len < sizeof(psd_obs->code.id))
		return ret;

	memcpy(id, psd_obs->code.id, sizeof(psd_obs->code.id));
...
static int plugin_misc_get_pid_name(struct plugin_misc *p, char *name, int len)
{
	...
	dev_info2(dev, "plugin_misc_get_pid_name name = %s, len = %d, pid_cfg->num_name = %d, obs->flag = 0x%08lx\n", name, len, pid_cfg->num_name, obs->flag);
	if (!name || len < 1)
		return ret;

	if (pid_cfg->num_name == 0)
		return ret;

	name[0] = '\0';
	if (!test_flag(MSC_FLAG_PSD_VALID, &obs->flag)) {
		if (!valid_config(pid_cfg->name_list[0].id, sizeof(pid_cfg->name_list[0].id), 0))
			idx = 0;
	}else {
		...
		if (i == pid_cfg->num_name) {
			if (!valid_config(pid_cfg->name_list[i -1].id, sizeof(pid_cfg->name_list[i -1].id), 0xFF)) {
				idx = i - 1;
			}
		}
	}

	if (idx >= 0 && idx < pid_cfg->num_name) {
		dev_info2(dev, "mxt get pid name %s\n", pid_cfg->name_list[idx].name);
		scnprintf(name, len, "%s", pid_cfg->name_list[idx].name);
		ret = 0;
	}
...
static long plugin_misc_post_process_messages(struct plugin_misc *p, unsigned long pl_flag)
{
	...
	if (test_flag(MSC_FLAG_RESET, &obs->flag)) {
		if (test_flag(MSC_FLAG_PTC_INITED, &obs->flag)) {
			...
			if (ret == 0) {
				ptc_obs->time_param_set = jiffies;
		...
		if (test_flag(MSC_STEP_PTC_SET_PARA, &ptc_obs->status.step)) {
			if (time_after_eq(jiffies, ptc_obs->time_param_set + ptc_cfg->interval_param_set_check)) {
				set_and_clr_flag(MSC_STEP_PTC_IDLE, MSC_STEP_PTC_MASK, &ptc_obs->status.step);
			}
		}
	}
		...
		if (test_flag(MSC_FLAG_PTC_TUNING, &obs->flag)) {	
			if (test_flag(MSC_STEP_PTC_IDLE, &ptc_obs->status.step)) {
				...
				if (ret == 0)
					...
					dev_err(dev, "mxt msc PTC tune failed %d\n", ret);
	...
	if (test_flag(MSC_STEP_PTC_SET_PARA, &ptc_obs->status.step))
		interval = ptc_obs->time_param_set + ptc_cfg->interval_param_set_check - jiffies + 1;
	return interval;
}
...
ssize_t plugin_misc_show(struct plugin_misc *p, char *buf, size_t count)
{
	...
	offset += misc_pid_show(p, buf + offset, count - offset);
				...
				if (strncmp(name, "psd", 3) == 0) {
					...
					if (ret == 1) {
						...
						if (strncmp(name, "get", 3) == 0) {
							...
							if (ret == 0) {
								...
								if (ret == 1 && val == 1) {
									...
									if (valid_config(obs->psd.code.id, sizeof(obs->psd.code.id), 0))
										...
										clear_flag(MSC_FLAG_PSD_VALID, &obs->flag);
								}
							}
						}else if (strncmp(name, "set", 3) == 0) {
								ret = sscanf(buf + offset, "%d: %n", &len, &ofs);
								if (len > 0 && len < sizeof(obs->psd.code.id)) {
									for (k = 0; k < len; k++) {
										offset += ofs;
										if (offset < count) {
											dev_info2(dev, "%s\n",buf + offset);
							...
							dev_info(dev, "set psd flag %ld\n",psd_obs->flag);
						}else if (strncmp(name, "store", 5) == 0) {
							dev_info(dev, "store psd data\n");
						...
						if (strncmp(name, "set", 6) == 0) {
							...
							if (test_flag(MSC_FLAG_PTC_INITED, &obs->flag)) {
								...
								if(val == 0) {  //tune and not store if not tuned before
									...
									dev_info(dev, "[mxt] obs->flag = 0x%08lx\n", obs->flag);
									if (!test_flag(MSC_FLAG_PTC_TUNED | MSC_FLAG_PTC_TUNING | MSC_FLAG_PTC_PARA_VALID, &obs->flag)) {
										set_and_clr_flag(MSC_FLAG_PTC_TUNING,MSC_FLAG_MASK_PTC, &obs->flag);
										if (test_flag(MSC_FLAG_PSD_VALID, &obs->flag))
											set_and_clr_flag(MSC_FLAG_PSD_UPDATING, MSC_FLAG_MASK_PSD, &obs->flag);
...
static int init_psd(struct plugin_misc *p)
{
	...
	if (ret == 0) {
		if (valid_config(psd_obs->code.id, sizeof(psd_obs->code.id), 0) &&
			valid_config(psd_obs->code.id, sizeof(psd_obs->code.id), 0xFF))
			set_flag(MSC_FLAG_PSD_VALID, &obs->flag);
	}
	return 0;
}
...
//*swf add*//
static struct pid_name pid_name_map_list[] = {
	//{"[cfg_postfix].[fw_postfix]",{0x0,0x0,0x0,0x0}}, 
    //BQ_L8450   Raw.01,02 point to truly v1；raw.03,raw.04 point to success; raw.05,raw.06 point to truly v2	
	//{"00.s",{0x0}},   //ZERO id is default for invalid PID, must be first
    {"01",{0x31,0x31,0x32,0x31,0x45,0xAA,0x00, 0x00}},
    {"02",{0x31,0x31,0x31,0x31,0x45,0xAA,0x00, 0x00}},
    {"03",{0x34,0x32,0x32,0x31,0x45,0xAA,0x00, 0x00}},
    {"04",{0x34,0x34,0x31,0x31,0x45,0xAA,0x00, 0x00}},
    {"05",{0x31,0x31,0x32,0x32,0x45,0xAA,0x00, 0x00}},
    {"06",{0x31,0x31,0x31,0x32,0x45,0xAA,0x00, 0x00}},    
	//{"04.t",{0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}},  //FF is for default unknow PID, must be last
};
...
static int init_ptc(struct plugin_misc *p, unsigned long pl_flag)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct device *dev = dcfg->dev;
	struct msc_config *cfg = p->cfg;
	...
	ptc_cfg->interval_param_set_check = 20;
	...
	if (test_flag(MSC_FLAG_PSD_VALID, &obs->flag)) {
		...
		if (ret == 0) {
			...
			if (!test_flag(PL_STATUS_FLAG_REG_INIT,&pl_flag))
...
static int init_msc_object(struct plugin_misc *p, unsigned long pl_flag)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct device *dev = dcfg->dev;
	int ret;
...
static int plugin_misc_msc_init(struct plugin_misc *p, unsigned long pl_flag)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct device *dev = dcfg->dev;
	...
	return init_msc_object(p, pl_flag);
}
...
struct plugin_misc mxt_plugin_misc = 
{
	...
	//.hook_reg_init = plugin_misc_hook_reg_init,
	.get_pid_val = plugin_misc_get_pid_val,
